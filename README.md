...
* Check the `ConnectionStrings` in `appsettings.json` files under the `PayrollPro.Web` and `PayrollPro.DbMigrator` projects and change it if you need.
...
```bash
dotnet dev-certs https -v -ep openiddict.pfx -p d41b0d1d-dcb4-4658-8a45-f86ab10dea34
```

> `d41b0d1d-dcb4-4658-8a45-f86ab10dea34` is the password of the certificate, you can change it to any password you want.
...
Run `PayrollPro.DbMigrator` to create the initial database. This should be done in the first run. It is also needed if a new database migration is added to the solution later.

### Solution structure

This is a layered monolith application that consists of the following applications:

* `PayrollPro.DbMigrator`: A console application which applies the migrations and also seeds the initial data. It is useful on development as well as on production environment.
* `PayrollPro.Web`: ASP.NET Core MVC / Razor Pages application that is the essential web application of the solution.
