...
Write-Host "=== Testing Company Registration -> User Creation ===" -ForegroundColor Cyan
...
try {
    ...
    if ($result.success) {
        ...
        if ($result.fullError) {
            Write-Host "Full error details: $($result.fullError)" -ForegroundColor Red
        }
        exit 1
    }
} catch {
...
Write-Host "✓ Company registration completed" -ForegroundColor Green
Write-Host "✓ User account creation verified" -ForegroundColor Green
